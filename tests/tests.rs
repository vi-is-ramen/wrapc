...
use wrapc::{LibrarySearchPathKind, LinkLibKind, parse};
...
fn codegen_and_z_flags() {
    ...
    assert_eq!(
        info.codegen_opts,
        vec!["opt-level=3", "link-arg=-fuse-ld=lld"]
    );
...
fn link_libraries_complex() {
    let a = args([
        ...
        "-lssl",
        "-lstatic=crypto",
        "-lstatic:+bundle,+whole-archive=mylib:renamed",
    ]);
    let info = parse(&a).unwrap();
